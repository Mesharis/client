...
// Get doctor and all its property
  ...
  Future<List<Doctor>> getTopRatedDoctor({int limit = 10}) async {
    try {
      var topRatedDoctor = await FirebaseFirestore.instance.collection('TopRatedDoctor').limit(limit).get();
  ...
  Future<List<Doctor>> searchDoctor(String doctorName) async {
    try {
      ...
      var doctorRef = await FirebaseFirestore.instance.collection('Doctors').where('doctorName',
  ...
  Future<String> getUserId(Doctor doctor) async {
    try {
      var user = await FirebaseFirestore.instance.collection('Users').where('doctorId', isEqualTo: doctor.doctorId).get();
